package app;

public class CacKieuModifier {
    /*
    Các kiểu Modifer trong Java
    Modifier là các từ khóa mà bạn thêm vào định nghĩa để thay đổi ý nghĩa của chúng,
    Ngôn ngữ Java có hàng loạt các Modifier, bao gồm:
    Access Modifier trong Java
    Non-Access Modifier trong Java
    Để sử dụng một Modifier, bạn bao từ khóa của nó định nghĩa của một lớp, phương thức
    hoặc biến. Modifier đặt trước phần còn lại của lệnh, như trong ví dụ sau:
    public class tenClass
    {
        //....
    }
    private boolean myFlag;
    static final double weeks = 9.5;
    protected staic final int BOXWIDTH = 42;
    public static void main(String [] arguments)
    {
        //Phan than phuong thuc
    }

    Access Modifier trong Java
    Java cung cấp một số Access Modifier để thiết lập chỉ định truy cập cho các lớp, các 
    biến, các phương thức và các constructor. Có 4 mức độ truy cập là:
    Default: Truy cập trong nội bộ package
    Private: Truy cập trong nội bộ lớp.
    Public: Thành phần công khai, truy cập tự do từ bên ngoài
    Protected: Thành phần được bảo vệ, bị hạn chế truy cập từ bên ngoài. 

    Non Access Modifier trong Java
    Java cũng cung cấp một số Non-Access Modifier để thực hiện các tính năng khác:
    Static Modifier để tạo các phương thức lớp và các biến.
    Final Modifier để kết thúc sự thi hành của các lớp, các phương thức và các biến.
    Abstract Modifier để tạo các lớp và các phương thức trừu tượng.
    Synchronized Modifier và Volatile Modifier được sử dụng cho các Thread.

    vietJack.com
    */
}